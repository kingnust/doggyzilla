...
class LineDetect:
    def __init__(self):
        ...
        self.Mouse_XY = (0, 0)
        text_path = '/home/pi/DOGZILLA/Samples/linefollow'
        self.hsv_text = text_path+"/ColorFollowHSV.text"
        self.PID_X = (50, 1, 20)
        self.PID_Y = (20, 1, 10)
        self.PID_init()
    ...
    def execute(self, point_x, point_y, radius):
        [X_Pid, _] = self.PID_controller_X.update([(point_x - 320), 0])
        [Y_Pid, _] = self.PID_controller_Y.update([(point_y - 120), 0])

        print("point_x:%d, point_y:%d, X_Pid:%d, Y_Pid:%d" % (point_x, point_y, int(X_Pid), int(-Y_Pid)))
        self.dog.attitude(['r','p'],[int(-X_Pid), int(-Y_Pid)])
    ...
    def PID_init(self):
        self.PID_controller_X = simplePID(
            [0, 0],
            [self.PID_X[0] / 1.0 / (self.scale), 0],
            [self.PID_X[1] / 1.0 / (self.scale), 0],
            [self.PID_X[2] / 1.0 / (self.scale), 0])
        self.PID_controller_Y = simplePID(
            [0, 0],
            [self.PID_Y[0] / 1.0 / (self.scale), 0],
            [self.PID_Y[1] / 1.0 / (self.scale), 0],
            [self.PID_Y[2] / 1.0 / (self.scale), 0])
